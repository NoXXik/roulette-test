...
function useNotificationCenter(params) {
  ...
  const sortFn = (0, import_react.useRef)(params.sort || defaultSort);
  const filterFn = (0, import_react.useRef)(params.filter || null);
  ...
  const notificationsRef = (0, import_react.useRef)(notifications);
  (0, import_react.useEffect)(() => {
    notificationsRef.current = notifications;
  }, [notifications]);
  (0, import_react.useEffect)(() => {
    return Q.onChange((toast) => {
      if (toast.status === "added" || toast.status === "updated") {
        const newItem = decorate(toast);
        if (filterFn.current && !filterFn.current(newItem))
          return;
  ...
  const add = (item) => {
    if (notificationsRef.current.find((v) => v.id === item.id))
      return null;
...
function decorate(item) {
  if (item.id == null)
    item.id = Date.now().toString(36).substring(2, 9);
  if (!item.createdAt)
...
//# sourceMappingURL=react-toastify_addons_use-notification-center.js.map
